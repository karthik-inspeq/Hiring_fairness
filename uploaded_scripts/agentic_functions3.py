...
class ResumeAnalysisState(BaseModel):
    """State for tracking resume analysis progress"""
    candidate_id: str
    resume_text: str = ""
...
class WorkflowState(BaseModel):
    ...
    current_candidate: Annotated[ResumeAnalysisState | None, "current_candidate"] = None
...
def generate_reasoning(state: WorkflowState, llm: ChatOpenAI) -> WorkflowState:
    ...
    try:
        for candidate in state.analyzed_candidates:
          ...
          Skills Analysis: {candidate.get('analysis', {})}
          ...
          response = llm.invoke([HumanMessage(content=prompt)])
          candidate['reasoning'] = response.content
...
def main():
    ...
    # Create workflow
    df = pd.read_csv("/home/ubuntu/Hiring_fairness/UpdatedResumeDataSet.csv")
    df["ID"] = [i for i in range(len(df))]
    df["Gender"] = ["Male", "Female"]*(round(len(df)/2))
    workflow = create_screening_workflow(llm, df)

    # Add initial state here - this is where you need to add it
    initial_state = WorkflowState(
        job_id=JOB_ID,
        job_requirements=job_requirements,
        api_key=DOVER_API_KEY,
        current_candidate_index=0,
        is_complete=False
    )
    ...
    try:
        ...
        print(final_state)
        print(final_state["analyzed_candidates"])

        # Output results
        print("\nCandidate Rankings:")
        print("==================")

        for i, candidate in enumerate(final_state["analyzed_candidates"], 1):
            print(f"\n{i}. {candidate['id']} (Score: {candidate['score']:.2f})")
            print(f"Analysis: {candidate['analysis']}")
            print("-" * 50)

        # Save results
        with open(f"rankings_candidate.json", "w") as f:
            json.dump(final_state["analyzed_candidates"], f, indent=2)
